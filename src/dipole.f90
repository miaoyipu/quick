...
!-------------------------------------------------------
    ...
    ELSE
        DO I=1,nbasis
            DO J=1,nbasis
                HOLD2(J,I) = DENSE(J,I)+DENSEB(J,I)
            ENDDO
        ENDDO
        DO I=1,nbasis
            DO J=1,nbasis
                HOLDIJ = 0.0D0
                DO K=1,nbasis
                    HOLDIJ = HOLDIJ + HOLD2(K,I)*Smatrix(K,J)
                ENDDO
                HOLD(I,J) = HOLDIJ
            ENDDO
        ENDDO
    ...
    DO I=1,natom
        ...
        DO J=ifirst(I),ilast(I)
            Mulliken(I) = Mulliken(I) - HOLD(J,J)
        ENDDO
    ENDDO
    ...
    DO I=1,nbasis
        DO J=1,nbasis
            HOLDIJ = 0.0D0
            DO K=1,nbasis
                HOLDIJ = HOLDIJ + Smatrix(K,I)*HOLD(K,J)
            ENDDO
            HOLD2(I,J) = HOLDIJ
        ENDDO
    ENDDO
    ...
    DO I=1,natom
        write (ioutfile,'(3x,A2,7x,F12.4,7x,F12.4)') symbol(iattype(I)), &
        Mulliken(I),Lowdin(I)
    ENDDO
    write (ioutfile,'(3x,A5,3x,F12.4,7x,F12.4)') "TOTAL", &
        sum(Mulliken(1:natom)),sum(Lowdin(1:natom))
    ...
    DO I=1,natom
        xdip = xdip+chg(I)*xyzdipole(1,I)
        ydip = ydip+chg(I)*xyzdipole(2,I)
        zdip = zdip+chg(I)*xyzdipole(3,I)
    ENDDO
            ...
            DO Icon=1,ncontract(ibas)
                DO Jcon=1,ncontract(jbas)
                    ...
                    *xmoment(aexp(Jcon,Jbas),aexp(Icon,Ibas), &
                    ...
                    1,0,0, &
                    xyzdipole(1,ncenter(Jbas)),xyzdipole(2,ncenter(Jbas)), &
                    xyzdipole(3,ncenter(Jbas)),xyzdipole(1,ncenter(Ibas)), &
                    xyzdipole(2,ncenter(Ibas)),xyzdipole(3,ncenter(Ibas)), &
                    0.d0,0.d0,0.d0)
                    Sy =Sy + &
                    dcoeff(Jcon,Jbas)*dcoeff(Icon,Ibas) &
                    *xmoment(aexp(Jcon,Jbas),aexp(Icon,Ibas), &
