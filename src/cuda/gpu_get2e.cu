                ...
                for (int KKK = 0; KKK < kPrimK; KKK++) {
                    ...
                    if (cutoffPrim2> devSim.primLimit) {
                        ...
                        switch (NABCDTYPE) {
                            ...
                            // PSSS orbital
                            case 10:
                            PSSS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx,(RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB);
                            break;
                            // SSPS orbital
                            case 1:
                            SSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            break;
                            // PSPS orbital
                            case 11:
                            PSSS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB);
                            SSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            SSPS(1, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            PSPS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*ABCD);
                            break;
                            
                            case 20:
                            PSSS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB);
                            PSSS(1, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB);
                            DSSS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*AB, (CC+DD)*ABCD);
                            break;
                            case 2:
                            SSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            SSPS(1, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            SSDS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD, 0.5*CD, (AA+BB)*ABCD);
                            break;
                            case 21:
                            SSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            ...
                            PSSS(2, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB);
                            DSSS(1, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*AB, (CC+DD)*ABCD);
                            DSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD, 0.5*ABCD);
                            break;
                            case 12:
                            SSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            ...
                            SSPS(2, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            SSDS(1, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD, 0.5*CD, (AA+BB)*ABCD);
                            PSDS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*ABCD);
                            break;
                            case 22:
                            SSPS(0, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            ...
                            SSPS(3, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD);
                            SSDS(2, YVerticalTemp,(RCx * CC + RDx * DD) * CD - RCx,(RCy * CC + RDy * DD) * CD - RCy,(RCz * CC + RDz * DD) * CD - RCz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RCx * CC + RDx * DD) * CD,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RCy * CC + RDy * DD) * CD,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RCz * CC + RDz * DD) * CD, 0.5*CD, (AA+BB)*ABCD);
                            PSDS(1, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*ABCD);
                            
                            PSPS(1, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*ABCD);
                            DSDS(0, YVerticalTemp, (RAx * AA + RBx * BB) * AB - RAx, (RAy * AA + RBy * BB) * AB - RAy, (RAz * AA + RBz * BB) * AB - RAz,(RAx * AA + RBx * BB + RCx * CC + RDx * DD) * ABCD - (RAx * AA + RBx * BB) * AB,(RAy * AA + RBy * BB + RCy * CC + RDy * DD) * ABCD - (RAy * AA + RBy * BB) * AB,(RAz * AA + RBz * BB + RCz * CC + RDz * DD) * ABCD - (RAz * AA + RBz * BB) * AB, 0.5*ABCD, 0.5*AB, (CC+DD)*ABCD);
                            break;
