...
/*
    for (int i = 0; i < gpu->natom; i++) {
        for (int j=0; j<3; j++) {
            printf("j=%i, i=%i, n=%i, xyz=%f \n", i,j,j*gpu->natom+i,(QUICKDouble) LOC2(gpu->xyz->_hostData,j,i,3,gpu->natom));
            //[j*gpu->natom+i]);
        }
    }
 */
...
extern "C" void gpu_upload_calculated_(QUICKDouble* o, QUICKDouble* co, QUICKDouble* vec, QUICKDouble* dense)
{
    ...
    for (int i = 0; i<gpu->nbasis; i++) {
        for (int j = 0; j<gpu->nbasis; j++) {
            QUICKULL valUII = (QUICKULL) (fabs ( LOC2( gpu->gpu_calculated->o->_hostData, i, j, gpu->nbasis, gpu->nbasis)*ENERGYSCALE + \
                                                 (QUICKDouble)0.5));
...
//    memset(gpu->gpu_calculated->oULL, 0ull, sizeof(QUICKULL)*gpu->nbasis*gpu->nbasis);
...
//    get2e_kernel<<<gpu->blocks, gpu->threadsPerBlock>>>();
...
#ifndef TEST
    dim3 blocks(gpu->gpu_basis->jshell, gpu->gpu_basis->jshell);
    dim3 threads(gpu->gpu_basis->jshell,gpu->gpu_basis->jshell);
    get2e_kernel<<<blocks,threads>>>();
#else
    ...
    for (int i = 0; i< gpu->nbasis; i++) {
        for (int j = i; j< gpu->nbasis; j++) {
            ...
            LOC2(gpu->gpu_calculated->o->_hostData,i,j,gpu->nbasis, gpu->nbasis) = (QUICKDouble)valDB*ONEOVERENERGYSCALE;
            LOC2(gpu->gpu_calculated->o->_hostData,j,i,gpu->nbasis, gpu->nbasis) = (QUICKDouble)valDB*ONEOVERENERGYSCALE;
        }
    }
