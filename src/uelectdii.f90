...
!    dimension alloperator(MAXDIISSCF,nbasis,nbasis)
    dimension alloperatorB(MAXDIISSCF,nbasis,nbasis)
    dimension B(MAXDIISSCF+1,MAXDIISSCF+1)
...
! alloperator array.)
    ...
    ! matrix hold.
        ...
        do I=1,nbasis
            do J=1,nbasis
                HOLDIJ = 0.0D0
                do K=1,nbasis
                    HOLDIJ = HOLDIJ + DENSE(K,I)*Smatrix(K,J)
                enddo
                HOLD(I,J) = HOLDIJ
            enddo
        enddo
    ...
    ! This means we now have the e(i) matrix.
        ...
        do I=1,nbasis
            do J=1,nbasis
                TEMPIJ = 0.0D0
                do K=1,nbasis
                    TEMPIJ = TEMPIJ + Smatrix(K,I)*HOLD(K,J)
                enddo
    ...
    ! First, calculate DENSEB*S and store in the scratch
    ! matrix hold.
        ...
        do I=1,nbasis
            do J=1,nbasis
                HOLDIJ = 0.0D0
                do K=1,nbasis
                    HOLDIJ = HOLDIJ + DENSEB(K,I)*Smatrix(K,J)
                enddo
                HOLD(I,J) = HOLDIJ
            enddo
        enddo

    ! Now calculate O*(DENSEB*S).  As the operator matrix is symmetric, the
    ...
    ! Calculate Db O.
        ...
        do I=1,nbasis
            do J=1,nbasis
                HOLDIJ = 0.0D0
                do K=1,nbasis
                    HOLDIJ = HOLDIJ + DENSEB(K,I)*O(K,J)
                enddo
                HOLD(I,J) = HOLDIJ
            enddo
        enddo
    ...
    ! 6) Store e'(i) in allerror.
        ...
        do I=1,nbasis
            do J=1,nbasis
                HOLDIJ = 0.0D0
                do K=1,nbasis
                    HOLDIJ = HOLDIJ + allerror(idiis,K,I) *X(K,J)
                enddo
                HOLD(I,J) = HOLDIJ
            enddo
        enddo
    ...
    ! |_                                             _||_  _|   |_  _|
        ...
        LSOLERR=0
        call LSOLVE(idiis+1,maxdiisscf+1,B,RHS,W,TOL,COEFF,LSOLERR)
    ...
    ! Density matrix to check for convergence.
        ...
        do I=1,nbasis
            do J=1,nbasis
                DENSEJI = 0.d0
                do K=1,nelecb
                    DENSEJI = DENSEJI + (COB(J,K)*COB(I,K))
                enddo
                DENSEB(J,I) = DENSEJI
            enddo
        enddo
    ...
    ! Now check for convergence.
        ...
        do I=1,nbasis
            do J=1,nbasis
                PRMS2=PRMS2+(DENSEB(J,I)-HOLD(J,I))**2.d0
                PCHANGE=MAX(PCHANGE,ABS(DENSEB(J,I)-HOLD(J,I)))
            enddo
        enddo
        ...
        50 continue
        diisdone = idiis.eq.MAXDIISSCF.or.diisdone
    enddo
