...
!    dimension alloperator(MAXDIISSCF,nbasis,nbasis)
    dimension alloperatorB(quick_method%maxdiisscf,nbasis,nbasis)
    dimension B(quick_method%maxdiisscf+1,quick_method%maxdiisscf+1)
...
! alloperator array.)
    ...
    ! Now diagonalize the operator matrix.
        ...
        CALL DIAG(nbasis,quick_qm_struct%o,nbasis,quick_method%DMCutoff,V2,EB,quick_qm_struct%idegen,quick_qm_struct%vec, &
        IERROR)
